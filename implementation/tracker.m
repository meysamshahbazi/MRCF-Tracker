...
%     end 
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            response_d{k1} = bsxfun(@times,beta_k .*  conj(cf_f{k1}), xtf{k1}); %��ͨ����Ӧ   ������beta
            scores_fs_feat{k1} = gather(sum(response_d{k1}, 3));
            scores_fs_sum = scores_fs_feat{k1};
            for k = block_inds
                scores_fs_feat{k} = gather(sum(bsxfun(@times, beta_k .* conj(cf_f{k}), xtf{k}), 3));% ������beta
            ...
            response_d{k1} = fftshift(ifft2(response_d{k1}, 'symmetric'));%��ͨ����Ӧ ʱ��
            ...
            shift_y = 1 - id_ymax_curr;%ֱ�Ӱ����ֵ��λ����1��1������yһ��
...
%             bar(reshape(real(beta_k), 1 ,[]))
    ...
    if(seq.frame == 1)
            % Compute convolution for each feature block in the Fourier domain
            % and the sum over all blocks.
            response_d{k1} = beta_k .* bsxfun(@times, conj(cf_f{k1}), xlf{k1}); % ÿ��ͨ����Ӧ��Ƶ����
            scores_fs_feat{k1} = gather(sum(response_d{k1}, 3));
            ...
            responsef_padded = resizeDFT2(scores_fs, output_sz);
            response = ifft2(responsef_padded, 'symmetric');
            response_d{k1} = ifft2(response_d{k1}, 'symmetric');% ��ͨ����Ӧ��תʱ��
            
            M_prev{1} = fftshift(response);
            Prev_response_d{1} = fftshift(response_d{k1});% ��ͨ����Ӧ
